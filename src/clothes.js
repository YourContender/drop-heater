import suit1 from "./helpers/images/suit-prod/man/1.jpg";
import suit2 from "./helpers/images/suit-prod/man/2.jpg";
import suit3 from "./helpers/images/suit-prod/man/3.jpg";
import suit4 from "./helpers/images/suit-prod/man/4.jpg";
import suit5 from "./helpers/images/suit-prod/man/5.jpg";
// import suit6 from "./helpers/images/suit-prod/man/6.jpg";
import suit7 from "./helpers/images/suit-prod/man/7.jpg";
import suit8 from "./helpers/images/suit-prod/man/8.jpg";
import suit9 from "./helpers/images/suit-prod/man/9.jpg";
import suit10 from "./helpers/images/suit-prod/man/10.jpg";
import suit11 from "./helpers/images/suit-prod/man/11.jpg";
import suit12 from "./helpers/images/suit-prod/man/12.jpg";
import suit13 from "./helpers/images/suit-prod/man/13.jpg";
import suit14 from "./helpers/images/suit-prod/man/14.jpg";
import suit15 from "./helpers/images/suit-prod/man/15.jpg";
import suit16 from "./helpers/images/suit-prod/man/16.jpg";
import suit17 from "./helpers/images/suit-prod/man/17.jpg";
import suit18 from "./helpers/images/suit-prod/man/18.jpg";
import suit19 from "./helpers/images/suit-prod/man/19.jpg";
import suit20 from "./helpers/images/suit-prod/man/20.jpg";

import w1 from "./helpers/images/suit-prod/woman/1.jpg";
import w2 from "./helpers/images/suit-prod/woman/2.jpg";
import w3 from "./helpers/images/suit-prod/woman/3.jpg";
import w4 from "./helpers/images/suit-prod/woman/4.jpg";
import w5 from "./helpers/images/suit-prod/woman/5.jpg";
import w6 from "./helpers/images/suit-prod/woman/6.jpg";
import w7 from "./helpers/images/suit-prod/woman/7.jpg";
import w8 from "./helpers/images/suit-prod/woman/8.jpg";
import w9 from "./helpers/images/suit-prod/woman/9.jpg";
import w10 from "./helpers/images/suit-prod/woman/10.jpg";
import w11 from "./helpers/images/suit-prod/woman/11.jpg";
import w12 from "./helpers/images/suit-prod/woman/12.jpg";
import w13 from "./helpers/images/suit-prod/woman/13.jpg";
import w14 from "./helpers/images/suit-prod/woman/14.jpg";
import w15 from "./helpers/images/suit-prod/woman/15.jpg";
import w16 from "./helpers/images/suit-prod/woman/16.jpg";
import w17 from "./helpers/images/suit-prod/woman/17.jpg";
import w18 from "./helpers/images/suit-prod/woman/18.jpg";
import w19 from "./helpers/images/suit-prod/woman/19.jpg";

import sizeMan from "./helpers/images/suit-prod/size-man.png";
import sizeWoman from "./helpers/images/suit-prod/size-woman.png";

// woman suit color: небесний 1-7
import customW1 from "./helpers/images/custom-prod/woman/1.jpg";
import customW2 from "./helpers/images/custom-prod/woman/2.jpg";
import customW3 from "./helpers/images/custom-prod/woman/3.jpg";
import customW4 from "./helpers/images/custom-prod/woman/4.jpg";
import customW5 from "./helpers/images/custom-prod/woman/5.jpg";
import customW6 from "./helpers/images/custom-prod/woman/6.jpg";
import customW7 from "./helpers/images/custom-prod/woman/7.jpg";

// woman suit color: бежевий 8-12
import customW8 from "./helpers/images/custom-prod/woman/8.jpg";
import customW9 from "./helpers/images/custom-prod/woman/9.jpg";
import customW10 from "./helpers/images/custom-prod/woman/10.jpg";
import customW11 from "./helpers/images/custom-prod/woman/11.jpg";
import customW12 from "./helpers/images/custom-prod/woman/12.jpg";

// woman suit color: темно синій 13-15
import customW13 from "./helpers/images/custom-prod/woman/13.jpg";
import customW14 from "./helpers/images/custom-prod/woman/14.jpg";
import customW15 from "./helpers/images/custom-prod/woman/15.jpg";

// woman suit color: чорний 16-20
import customW16 from "./helpers/images/custom-prod/woman/16.jpg";
import customW17 from "./helpers/images/custom-prod/woman/17.jpg";
import customW18 from "./helpers/images/custom-prod/woman/18.jpg";
import customW19 from "./helpers/images/custom-prod/woman/19.jpg";
import customW20 from "./helpers/images/custom-prod/woman/20.jpg";

// woman suit color: шоколад 21-24
import customW21 from "./helpers/images/custom-prod/woman/21.jpg";
import customW22 from "./helpers/images/custom-prod/woman/22.jpg";
import customW23 from "./helpers/images/custom-prod/woman/23.jpg";
import customW24 from "./helpers/images/custom-prod/woman/24.jpg";

// woman suit color: мокко 25-31
import customW25 from "./helpers/images/custom-prod/woman/25.jpg";
import customW26 from "./helpers/images/custom-prod/woman/26.jpg";
import customW27 from "./helpers/images/custom-prod/woman/27.jpg";
import customW28 from "./helpers/images/custom-prod/woman/28.jpg";
import customW29 from "./helpers/images/custom-prod/woman/29.jpg";
import customW30 from "./helpers/images/custom-prod/woman/30.jpg";
import customW31 from "./helpers/images/custom-prod/woman/31.jpg";

// woman suit color: молочний 32-38
import customW32 from "./helpers/images/custom-prod/woman/32.jpg";
import customW33 from "./helpers/images/custom-prod/woman/33.jpg";
import customW34 from "./helpers/images/custom-prod/woman/34.jpg";
import customW35 from "./helpers/images/custom-prod/woman/35.jpg";
import customW36 from "./helpers/images/custom-prod/woman/36.jpg";
import customW37 from "./helpers/images/custom-prod/woman/37.jpg";
import customW38 from "./helpers/images/custom-prod/woman/38.jpg";

// 19.12 new design

export const cardsProductsWomanSuits = [
	{
		id: 1,
		images: [
			customW5,
			customW1,
			customW2,
			customW3,
			customW4,
			customW6,
			customW7,
		],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "небесний",
		backgroundColor: "#8B97A7",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 2,
		images: [customW8, customW9, customW10, customW11, customW12],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "бежевий",
		backgroundColor: "#AE9B9D",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 6,
		images: [
			customW25,
			customW29,
			customW26,
			customW27,
			customW28,
			customW30,
			customW31,
		],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "мокко",
		backgroundColor: "#836356",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 3,
		images: [customW13, customW14, customW15],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "т.синій",
		backgroundColor: "#161F2E",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 4,
		images: [customW16, customW17, customW18, customW19, customW20],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "чорний",
		backgroundColor: "#040507",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 5,
		images: [customW21, customW22, customW23, customW24],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "шоколад",
		backgroundColor: "#423333",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 7,
		images: [
			customW32,
			customW33,
			customW34,
			customW35,
			customW36,
			customW37,
			customW38,
		],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "молочний",
		backgroundColor: "#E2E3DE",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
];

// custom

export const cards = [
	{
		id: 1,
		images: [
			customW5,
			customW1,
			customW2,
			customW3,
			customW4,
			customW6,
			customW7,
		],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "небесний",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 2,
		images: [customW8, customW9, customW10, customW11, customW12],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "бежевий",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 6,
		images: [
			customW25,
			customW29,
			customW26,
			customW27,
			customW28,
			customW30,
			customW31,
		],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "мокко",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 3,
		images: [customW13, customW14, customW15],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "т.синій",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 4,
		images: [customW16, customW17, customW18, customW19, customW20],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "чорний",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 5,
		images: [customW21, customW22, customW23, customW24],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "шоколад",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
	{
		id: 7,
		images: [
			customW32,
			customW33,
			customW34,
			customW35,
			customW36,
			customW37,
			customW38,
		],
		oldPrice: 2070,
		price: 1099,
		sale: "47%",
		color: "молочний",
		sizeTop: ["S", "M", "L", "XL"],
		sizeBottom: ["2XL", "3XL", "4XL"],
		material: "двосторонній фліс (Polar)",
	},
];

//=================== ROBE

export const cardsProductsManRobe = null;

export const imagesForRobeManSlider = [
	suit1,
	suit2,
	suit3,
	suit4,
	suit5,
	// suit6,
	suit7,
	suit8,
	suit9,
	suit10,
	suit11,
	suit12,
	suit13,
	suit14,
	suit15,
	suit16,
	suit17,
	suit18,
	suit19,
	suit20,
];

export const prodTitleRobeMan = "Чоловічий теплий халат преміум якості ";

export const costRobeMan = {
	old: 2250,
	new: 1099,
};

export const infoRobeMan =
	"Костюми виконані із натурального щільного матеріалу, який буде зігрівати щодня, м'яка плюшева тканина подарує затишок у будь-яку погоду. З нашим костюмом ві будете виглядати сучасно та впевнено. Ідеально підходить як для прогулянок, так і для дому.";

export const detailsTitleRobeMan = "Опис";

export const detailsForRobeMan = [
	{
		title: "Матеріал: ",
		descr: "тканина махра welsoft",
	},
	{
		title: "Розмір: ",
		descr: "L, XL, 2XL, 3XL",
	},
	{
		title: "мʼякий та приємний до тіла",
		descr: "",
	},
	{
		title: "халат з кишенями, поясом та капюшоном",
		descr: "",
	},
];

export const equipmentForRobeMan = [
	"🔥 Костюм складається зі стильного худі та зручних штанів із манжетами",
	"🔥 Легко підійде для активного відпочинку, прогулянок чи домашнього затишку",
	"🔥 Мінімалістичний, універсальний стиль, що підходить для будь-якого віку",
	"🔥 Кольори, які гармонійно поєднуються з іншими елементами гардероба",
	"🔥 Великий вибір розмірів – ідеально сідає на будь-яку фігуру",
	"🔥 Тканина стійка до зносу і прання, не втрачає вигляд навіть після багаторазового використання",
];

export const equipmentTitleForRobeMan = "Детальніше";

export const reviewsForRobeMan = [
	{
		rating: "5",
		name: "Віта",
		text: "Дужеее теплий, рекомендую😍😍😍",
	},
	{
		rating: "5",
		name: "Володимир",
		text: "Розмір підійшов просто ідеально, дуже комфортний",
	},
	{
		rating: "5",
		name: "Аліна",
		text: "Саме те що треба, зігріває та приємний на дотик 😘💋",
	},
	{
		rating: "5",
		name: "Єва",
		text: "Замовила собі і чоловіку, тепер ми як два ведмедика) клас, рекомендую 👍🏻",
	},
	{
		rating: "5",
		name: "Ігор",
		text: "Дякую",
	},
];

export const marqueeRobeMan = "ВСТИГНИ ПРИДБАТИ НАБІР ЗІ ЗНИЖКОЮ ВІД ";

export const sizeForRobeMan = [
	{
		sm: "S",
		old: 2070,
		new: 1099,
	},
	{
		sm: "M",
		old: 2070,
		new: 1099,
	},
	{
		sm: "L",
		old: 2070,
		new: 1099,
	},
	{
		sm: "XL",
		old: 2070,
		new: 1099,
	},
	{
		sm: "2XL",
		old: 2070,
		new: 1099,
	},
];

export const colorForRobeMan = [];

export const ten = null;

// =======================================================================

export const imagesForSuitWomanSlider = [
	w1,
	w2,
	w3,
	w4,
	w5,
	w6,
	w7,
	w8,
	w9,
	w10,
	w11,
	w12,
	w13,
	w14,
	w15,
	w16,
	w17,
	w18,
	w19,
];

export const prodTitleSuitWoman =
	"Жіночий теплий плюшевий костюм преміум якості ";

export const costSuitWoman = {
	old: 2070,
	new: 1099,
};

export const infoSuitWoman =
	"Костюми виконані із натурального щільного матеріалу, який буде зігрівати щодня, м'яка плюшева тканина подарує затишок у будь-яку погоду. З нашим костюмом ві будете виглядати сучасно та впевнено. Ідеально підходить як для прогулянок, так і для дому.";

export const detailsTitleSuitWoman = "Опис";

export const detailsForSuitWoman = [
	{
		title: "Матеріал: ",
		descr: "двосторонній фліс (Polar)",
	},
	{
		title: "Розмір: ",
		descr: "S M L XL 2XL 3XL 4XL",
	},
	{
		title: "Комфортна та зручна",
		descr: "",
	},
	{
		title: "Кольори:",
		descr: "7 кольорів",
	},
	{
		title: "Базовий крій, підійде на будь-яку фігуру",
		descr: "",
	},
];

export const equipmentForSuitWoman = [
	"🔥 Костюм складається зі стильного худі та зручних штанів із манжетами",
	"🔥 Легко підійде для активного відпочинку, прогулянок чи домашнього затишку",
	"🔥 Мінімалістичний, універсальний стиль, що підходить для будь-якого віку",
	"🔥 Кольори, які гармонійно поєднуються з іншими елементами гардероба",
	"🔥 Великий вибір розмірів – ідеально сідає на будь-яку фігуру",
	"🔥 Тканина стійка до зносу і прання, не втрачає вигляд навіть після багаторазового використання",
];

export const equipmentTitleForSuitWoman = "Детальніше";

export const reviewsForSuitWoman = [
	{
		rating: "5",
		name: "Віта",
		text: "Дужеее теплий, рекомендую😍😍😍",
	},
	{
		rating: "5",
		name: "Аліна",
		text: "Ваааааау 💋💋💋",
	},
	{
		rating: "5",
		name: "Ліза",
		text: "Ну який же він теплииииий😘",
	},
	{
		rating: "5",
		name: "Єва",
		text: "Замовила два кольори, дуже гарні, дякую вам!",
	},
	{
		rating: "5",
		name: "Тетяна",
		text: "Якісно, дуже якісно ",
	},
	{
		rating: "5",
		name: "Діана",
		text: "Мені прям дуже сподобалось, просто шик",
	},
	{
		rating: "5",
		name: "Юля",
		text: "Дуууже теплий😍 10/10 однозначно",
	},
];

export const marqueeSuitWoman = "ВСТИГНИ ПРИДБАТИ НАБІР ЗІ ЗНИЖКОЮ ВІД ";

export const sizeForSuitWoman = [
	{
		sm: "S",
		old: 2070,
		new: 1099,
	},
	{
		sm: "M",
		old: 2070,
		new: 1099,
	},
	{
		sm: "L",
		old: 2070,
		new: 1099,
	},
	{
		sm: "XL",
		old: 2070,
		new: 1099,
	},
	{
		sm: "2XL",
		old: 2070,
		new: 1099,
	},
	{
		sm: "3XL",
		old: 2070,
		new: 1099,
	},
	{
		sm: "4XL",
		old: 2070,
		new: 1099,
	},
];

export const colorForSuitWoman = [
	{
		sm: "небесний",
	},
	{
		sm: "шоколад",
	},
	{
		sm: "чорний",
	},
	{
		sm: "мокко",
	},
	{
		sm: "молочний",
	},
	{
		sm: "бежевий",
	},
	{
		sm: "т. синій",
	},
];

export const tenWoman = sizeWoman;
